param(
  ...
  [string] $OutputRoot = "artifacts\uitoolkit-tests"
)
...
if (-not $process.WaitForExit([int][TimeSpan]::FromMinutes(10).TotalMilliseconds)) {
  ...
  throw "EveUnity UI Toolkit tests timed out after 10 minutes (Unity PID $($process.Id))."
}
$unityExitCode = $process.ExitCode
if ($unityExitCode -ne 0) {
  if (Test-Path -LiteralPath $logPath) { Get-Content -LiteralPath $logPath -Tail 120 }
  throw "EveUnity UI Toolkit tests failed with exit code $unityExitCode"
}
...
if ($null -eq $run -or [int]$run.total -eq 0 -or [int]$run.failed -gt 0) {
  throw "EveUnity UI Toolkit tests did not pass: $resultsPath"
}
Write-Host "EveUnity generic UI Toolkit tests: $($run.passed) passed, $($run.total) total"
